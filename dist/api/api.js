...
export class APIService {
    constructor() {
        this.locationService = new LocationWeatherService();
        this.cache = new CacheService();
        this.pendingRequests = new Map();
    }
    get expectedSpriteSheetSize() {
        return CONFIG.TILE_SIZE * 4;
    }
    get debugMode() {
    ...
    async generateSpriteSheet(dogDescription, imageBase64) {
        ...
        const expectedSize = CONFIG.TILE_SIZE * CONFIG.SPRITE_SHEET_WIDTH;
    ...
    async _doGenerateEnemySpriteSheet(enemyType) {
    ...
    async _doGenerateLocationBackground(progressCallback) {
    ...
    _getFallbackBackgroundDescription() {
        const fallbacks = [
    ...
    async _generateBackgroundFrames(description, timeWeather, progressCallback) {
        ...
        for (let i = 0; i < totalFrames; i++) {
            const frameNum = i + 1;
            const framePrompt = PromptTemplates.framePrompt(animationContext, description, frameNum, totalFrames, hasPrecipitation, precipitationType);
