...
export class Game {
    ...
    // Background
    private backgroundSprites: PhaserSprite[] = [];
    private backgroundFrameTimer: PhaserTimerEvent | null = null;
    ...
    constructor(
        ...
        this.config = {
            ...
            scene: {
                preload: function (this: PhaserScene): void {
                    self.preload(this);
                },
                create: function (this: PhaserScene): void {
                    self.create(this);
                },
                update: function (this: PhaserScene): void {
                    self.update(this);
                },
            },
            pixelArt: true,
    ...
    private async create(scene: PhaserScene): Promise<void> {
        try {
            ...
            // Create level (platforms, collectibles)
            ...
            this.enemyManager.setCallbacks({
                onEnemyKilled: (enemy: Enemy, player: PhaserSprite) => this.onEnemyKilled(enemy, player),
                onPlayerHit: (enemy: Enemy, player: PhaserSprite) => this.onPlayerHit(enemy, player),
            });
    ...
    private handleCriticalAssetError(message: string): void {
        console.error('CRITICAL ASSET ERROR:', message);

        errorHandler.createError(
            ErrorType.ASSET_LOAD_ERROR,
            message,
            { operation: 'game_asset_load', module: 'Game' }
        );
    ...
    private async handlePlayerSpriteSetup(scene: PhaserScene): Promise<void> {
        ...
        let processedImage: HTMLImageElement = playerTexture.source[0].image;
        ...
        (scene as any).playerSpriteScale = scaleFactor;
        (scene as any).playerSpriteNeedsScaling = true;
        (scene as any).playerFrameWidth = frameWidth;
        (scene as any).playerFrameHeight = frameHeight;
    ...
    private setupPlayerPhysics(scene: PhaserScene): void {
        if (!scene.textures.exists('playerSprite')) {
        ...
        if (this.player.body) {
            ...
            this.player.body.setOffset(
                ((scene as any).playerFrameWidth - bodyWidth) / 2,
                (scene as any).playerFrameHeight - bodyHeight
            );
        }
    ...
    private async createLevel(scene: PhaserScene, level: number): Promise<void> {
    ...
    private createPlatforms(scene: PhaserScene, level: number): void {
    ...
    private createCollectibles(scene: PhaserScene, level: number): void {
    ...
    private setupInput(scene: PhaserScene): void {
    ...
    private handlePlayerInput(isOnGround: boolean): void {
    ...
    private createAnimations(scene: PhaserScene): void {
        if (!scene.textures.exists('playerSprite')) return;

        const createAnim = (key: string, start: number, end: number, frameRate: number): void => {
        ...
        // Row 2: Jump (frames 8-11)
        // Row 3: Idle (frames 12-15)
    ...
    private onEnemyKilled(enemy: Enemy, _player: PhaserSprite): void {
    ...
    private onPlayerHit(enemy: Enemy, _player: PhaserSprite): void {
    ...
    private collectItem(_player: PhaserSprite, item: PhaserSprite): void {
    ...
    private loseLife(damage: number = 1): void {
    ...
    private showLevelTransition(text: string, callback: () => void): void {
    ...
    private showOverlay(title: string, subtitle: string, color: string): void {
        const overlay = document.createElement('div');
    ...
    public async loadLocationBackground(scene: PhaserScene): Promise<void> {
        let frames: string[] = [];
    ...
    private loadBase64Texture(scene: PhaserScene, key: string, base64: string): Promise<void> {
    ...
    private startBackgroundAnimation(scene: PhaserScene): void {
        ...
        this.backgroundFrameTimer = scene.time.addEvent({
            ...
            loop: true,
            callback: () => {
                if (!this.backgroundSprites || this.backgroundSprites.length === 0) return;

                this.backgroundSprites.forEach((bg) => {
                    if (!bg || !bg.active) return;

                    const current = bg.getData('frameIndex') as number;
    ...
    private createPlatformTexture(scene: PhaserScene): void {
