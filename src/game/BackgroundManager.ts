/**
 * game/BackgroundManager.ts
 * Handles background loading and animation
 * Animates through 8 individual frames for a seamless looping background
 */

import { CONFIG } from '../config.js';
import type { AssetStorage } from '../AssetStorage.js';
import { PhaserScene, PhaserSprite, PhaserTimerEvent } from './types.js';

export class BackgroundManager {
    private scene: PhaserScene;
    private assetStorage: AssetStorage;
    private backgroundTiles: PhaserSprite[] = [];
    private frameTimer: PhaserTimerEvent | null = null;
    private isUpdating: boolean = false;
    private currentFrameIndex: number = 0;
    private totalFrames: number = 8;

    constructor(scene: PhaserScene, assetStorage: AssetStorage) {
        this.scene = scene;
    ...
    public async load(): Promise<void> {
        ...
        // Create background tiles that span the world
    ...
    private async getFramesFromStorage(): Promise<string[]> {
        let frames: string[] = [];
    ...
    private async loadFrameTextures(frames: string[]): Promise<number> {
        let loadedCount = 0;
    ...
    private loadBase64Texture(key: string, base64: string): Promise<void> {
    ...
    private createBackgroundTiles(): void {
        ...
        // Check if first frame texture exists
    ...
    private startAnimation(): void {
        ...
        // Calculate delay based on animation speed (frames per second)
    ...
    private advanceFrame(): void {
        ...
        // Update all tiles to show the new frame
