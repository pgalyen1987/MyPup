...
export const ColorUtils = {
    ...
    isBackgroundColor(color: RGB, target: RGB, tolerance: number = API_CONSTANTS.COLOR.BACKGROUND_TOLERANCE): boolean {
    ...
    sampleEdgePixels(data: Uint8ClampedArray, width: number, height: number): RGB[] {
        const samples: RGB[] = [];
        ...
        for (let i = 0; i < API_CONSTANTS.EDGE_SAMPLES; i++) {
            corners.push(
                { x: Math.floor((width / API_CONSTANTS.EDGE_SAMPLES) * i), y: 0 },
                { x: Math.floor((width / API_CONSTANTS.EDGE_SAMPLES) * i), y: height - 1 },
                { x: 0, y: Math.floor((height / API_CONSTANTS.EDGE_SAMPLES) * i) },
                { x: width - 1, y: Math.floor((height / API_CONSTANTS.EDGE_SAMPLES) * i) }
            );
        }
    ...
    detectBackgroundColor(samples: RGB[]): RGB {
